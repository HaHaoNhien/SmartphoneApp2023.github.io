...
const elementBack = document.getElementById('btn99')
...
num1.addEventListener('click', function () {
  ...
  soundControl('start', 'sound/num.mp3')
  soundEndflag = '1'
...
elementMult.addEventListener('click', function () {
  //sound control
  if (soundEndflag === '1') {
    soundControl('end', '')
  }
...
elementEqual.addEventListener('click', function () {
  ...
  soundControl('start', 'sound/equal.mp3')
  soundEndflag = '1'
})
elementCancel.addEventListener('click', function () {
  ...
  soundControl('start', 'sound/cancel.mp3')
  soundEndflag = '1'
})
...
elementBack.addEventListener('click', function () {
  ...
  soundControl('start', 'sound/back.mp3')
  soundEndflag = '1'
  ...
  // １つ前の入力が演算子（演算子 入力しなおし）
    ...
    if (wkFirst === '1') {
      ...
      if (['+', '-', '*', '/'].includes(wkLogLastWord)) {
        elementcalcLog.innerHTML =
          elementcalcLog.innerHTML.slice(0, -1) + calcType //計算ログ　末尾1文字（前回の演算子）を削除
      }
      // イコールの後の演算子
      else {
